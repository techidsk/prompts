...
export default function HistoryPage() {
  ...
  // 加载到 Playground
  const handleLoadToPlayground = (record: HistoryRecord) => {
    navigate({
      to: "/",
      search: {
        historyId: record.id,
    ...
    <div className="flex h-full">
      ...
      <aside className="w-96 bg-surface-900/50 backdrop-blur-sm border-r border-surface-800 flex flex-col">
        ...
        <div className="p-4 border-b border-surface-800">
          ...
          <p className="text-xs text-surface-500 mt-1">共 {total} 条记录</p>
        </div>
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-surface-500">
              ...
              <p className="text-xs mt-1">开始对话后记录会自动保存</p>
            </div>
          ) : (
      ...
      <main className="flex-1 bg-surface-950 overflow-auto">
        {selectedRecord ? (
          <div className="max-w-4xl mx-auto p-8">
            ...
            <div className="mb-6 pb-6 border-b border-surface-800">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => handleLoadToPlayground(selectedRecord)}
                  className="px-4 py-2 bg-accent hover:bg-accent-light text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col items-center justify-center h-full text-surface-500">
            ...
            <p className="text-lg">选择一条记录查看详情</p>
            <p className="text-sm mt-1 text-surface-600">点击左侧列表中的记录</p>
          </div>
        )}
      </main>
    </div>
